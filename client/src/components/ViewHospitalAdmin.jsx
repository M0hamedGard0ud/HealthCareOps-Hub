...
console.log(token);
    ...
    const fetchHospitalData = async () => {
      try {
        ...
        // Assuming 'vendoremail' is the key in cookies
         ...
         const filteredUser = data.filter((user) => user.role == 'Hospital' );
         setHospitalData(filteredUser);
            setLoading(false);
      } catch (error) {
  ...
  const filteredData = hospitalData.filter(user => 
                  ...
                  <div key={user._id} className="col-12 col-md-6">
                    <div className="card product-card" style={{marginBottom:10}}>
                      <div className="card-body">
                        ...
                        <a className="product-title d-block" style={{color:'green'}}>Status: {user.status}</a>
                      </div>
                    </div>   
...
export default ViewHospitalAdmin;
